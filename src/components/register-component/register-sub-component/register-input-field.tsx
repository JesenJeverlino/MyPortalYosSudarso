...
export default function RegisterInputField({ label, inputType, size, name }: propsType) {
  ...
  return (
    <>
      ...
      {inputType === "password" ? (
          ...
          <input {...register(name)} type={showPassword ? "text" : "password"} className={`bg-[#e8e8e8] w-full h-full pl-4`}/>
        ...
        <input {...register(name)} type={inputType} className={`bg-[#e8e8e8] h-[40px] pl-4 ${currentSize.input}`}/>
      )}
    </>
  );
}